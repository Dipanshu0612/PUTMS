  ...
  useEffect(() => {
    async function getUserInfo() {
      let response = await axios.post(
        "https://putms.onrender.com/getUserInfo",
        { user_id },
  ...
  return (
    <>
      ...
      <div className="w-full h-[30rem] flex px-2 bg-slate-200">
        <div
          className="w-2/3 m-3 bg-slate-100 p-3 cursor-pointer shadow-lg"
          ref={targetRef}
        >
            ...
            <div className="h-[16.5rem] flex p-2 space-x-2">
              <div className="w-1/3">
                <img
                  ...
                  alt="UserIMG"
                  className="border border-black object-cover rounded-lg mix-blend-multiply"
                />
              </div>
              <div className="w-2/3 flex p-2 tracking-wide text-4xl gap-4 relative mb-1">
                <div className="flex flex-col space-y-5 justify-center mb-5">
                  ...
                  <h6 className="font-bold">
                    Enrollment : {userData.Enrollment || userData.MIS_ID}
                  </h6>
                  <h6 className="font-bold">Card ID : {userData.Card_ID}</h6>
                  <h6 className="font-bold">
                    Bus Pass No : {userData.Bus_Pass_No}
                  </h6>
                  <h6 className="font-bold">
        ...
        <div className="w-1/3 m-3 bg-slate-100 p-3 flex flex-col space-y-1 shadow-lg cursor-pointer">
          ...
          <div className="flex">
            <button
              className="flex flex-col bg-blue-500 items-center justify-center px-[1.4rem] py-4 m-2 hover:bg-blue-800 ease-in-out transition font-bold hover:text-white rounded-md duration-200"
              onClick={() => toPDF()}
            >
            ...
            <button
              className={`flex flex-col bg-green-500 items-center justify-center px-[3rem] py-4 font-bold text-xl m-2 hover:bg-green-700 hover:text-white rounded-md transition ease-in-out duration-200 disabled:cursor-not-allowed`}
